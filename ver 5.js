// ==UserScript==
// @name         🏝️ Time Island & Sidebar Widgets v5 (Hybrid)
// @namespace    https://achma-learning.github.io/
// @version      5.0.1
// @description  Floating island with clock, dates (EN/Hijri), prayer countdown, live age + sidebar: prayer times (35 Moroccan cities, calibrated against habous.gov.ma), weather, calendar, life-in-weeks grid (10-stage chronology or classic canvas grid, toggleable), live age counter, stopwatch (rAF-driven), notes, editable links (URL-validated). Auto-hide via macOS-style edge proximity, section toggles, scale/font/blur/color presets, prayer glow. Alt+Ctrl=sidebar, Alt+T=island, Alt+C=city search, Alt+Ctrl+Space=command palette. Drag-to-Custom position persisted across reloads. Hardened: SPA/iframe re-injection guard, rel=noopener on external links, percent-encoded weather city, request-token guards against stale fetches, palette closes on outside click, Hijri cached per-day.
// @author       Achma
// @match        *://*/*
// @grant        GM_addStyle
// @grant        GM_getValue
...
// @run-at       document-end
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/571855/%F0%9F%8F%9D%EF%B8%8F%20Time%20Island%20%20Sidebar%20Widgets%20v4.user.js
// @updateURL https://update.greasyfork.org/scripts/571855/%F0%9F%8F%9D%EF%B8%8F%20Time%20Island%20%20Sidebar%20Widgets%20v4.meta.js
// ==/UserScript==
...
(function () {
  ...
  const WICON={
    ...
    'Patchy rain possible':'🌦️','Patchy light rain':'🌦️','Patchy light drizzle':'🌦️','Light drizzle':'🌦️',
    'Light rain':'🌧️','Rain':'🌧️','Moderate rain':'🌧️','Moderate rain at times':'🌧️',
    'Heavy rain':'🌧️','Heavy rain at times':'🌧️',
    'Light rain shower':'🌧️','Moderate or heavy rain shower':'🌧️','Torrential rain shower':'🌧️',
    'Thunderstorm':'⛈️','Thundery outbreaks possible':'⛈️','Patchy light rain with thunder':'⛈️','Moderate or heavy rain with thunder':'⛈️',
    'Snow':'❄️','Light snow':'🌨️','Moderate snow':'❄️','Heavy snow':'❄️',
    'Patchy light snow':'🌨️','Patchy moderate snow':'❄️','Patchy heavy snow':'❄️',
    'Light snow showers':'🌨️','Moderate or heavy snow showers':'❄️',
    'Blowing snow':'🌬️❄️','Blizzard':'🌬️❄️',
    'Sleet':'🌨️','Light sleet':'🌨️','Moderate or heavy sleet':'🌨️','Light sleet showers':'🌨️','Moderate or heavy sleet showers':'🌨️',
    'Freezing drizzle':'🧊','Heavy freezing drizzle':'🧊',
    'Light freezing rain':'🧊','Moderate or heavy freezing rain':'🧊',
    'Ice pellets':'🧊','Light showers of ice pellets':'🧊','Moderate or heavy showers of ice pellets':'🧊'
  };
  ...
  function pickWeatherEmojiPlain(desc){
    return desc?(WICON[desc]||'🌤️'):'🌤️';
  }
  ...
  // Standard Gregorian date format used everywhere: "23 July 2026" — no
  // weekday, no "الموافق" prefix, just day + full month name + year.
  const enDateStr=d=>`${d.getDate()} ${EN_M[d.getMonth()]} ${d.getFullYear()}`;
  const isSafeUrl=u=>{try{const p=new URL(u,location.href);return p.protocol==='http:'||p.protocol==='https:'}catch{return false}};
  ...
  let prayerReqId=0, weatherReqId=0;      // bumped on every fetch; stale callbacks are dropped
  ...
  const cfg = {
    ...
    useOldLifeCal: gGet('ti_useOldLifeCal', false),   // false = 10-stage chronology grid, true = plain canvas grid
  ...
  // ── Life Calendar: 10-stage neurological / Islamic-age chronology ──
  // Each band is an [ageStart, ageEnd) range. Bounds follow the more specific
  // breakdown (e.g. 18–25 rather than the looser "16–25 jeunesse") since it
  // lines up with concrete markers: Tamyeez (discernment) at 7, Taklif
  // (religious accountability) at 10–14, legal majority at 18, prefrontal
  // maturation finishing around 25, the Quranic "ashuddahu" (full strength)
  // window at 40–60, and the Prophetic median lifespan hadith at 60–70.
  const LIFE_STAGES=[
    {min:0,  label:'0–2 Suckling',  color:'#FADADD'}, // Quranic nursing period — sensorimotor explosion
    {min:2,  label:'2–7 Fitrah',    color:'#F3E5AB'}, // natural, unshaped disposition; language acquisition
    {min:7,  label:'7–10 Tamyeez',  color:'#A8C3A6'}, // discernment; peak synaptic pruning
    {min:10, label:'10–14 Taklif',  color:'#F39C12'}, // religious accountability begins; limbic overdrive
    {min:14, label:'14–18 Identity',color:'#FF6F61'}, // identity formation; legal minority
    {min:18, label:'18–25 Forging', color:'#FFD700'}, // prefrontal finalization; peak fluid intelligence
    {min:25, label:'25–40 Ascend',  color:'#4A90E2'}, // peak crystallized intelligence, stable building
    {min:40, label:'40–60 Wisdom',  color:'#B87333'}, // Quranic "ashuddahu" — full strength / wisdom
    {min:60, label:'60–70 Median',  color:'#9B59B6'}, // Prophetic median lifespan of the Ummah
    {min:70, label:'70–80 Grace',   color:'#BDC3C7'}, // Harim — statistically rare, blessed overtime
  ];
  function lifeStage(age){
    for(let i=LIFE_STAGES.length-1;i>=0;i--) if(age>=LIFE_STAGES[i].min) return LIFE_STAGES[i];
  ...
  userLinks=userLinks.filter(lk=>lk&&isSafeUrl(lk.u));
  ...
  // ═══════════════════════════════════════════
  GM_addStyle(`
:root{--ti:rgba(15,23,42,.88);--tig:rgba(255,255,255,.06);--tib:rgba(255,255,255,.10);--tit:#e2e8f0;--tid:#94a3b8;--tia:#38bdf8;--tia2:#a78bfa;--tig2:#34d399;--tir:#dc2626;--tio:#fb923c;--tip:#af52de;--tif:-apple-system,BlinkMacSystemFont,'SF Pro Text','Segoe UI',system-ui,sans-serif;--tim:'SF Mono','Cascadia Code','Fira Code',monospace;--tiar:'Amiri','Traditional Arabic','Noto Naskh Arabic',serif}
...
.ti-wp-row{display:grid;grid-template-columns:78px 28px auto 1fr auto;gap:10px;align-items:center;padding:8px 4px;border-top:1px dashed rgba(255,255,255,.08);margin-top:8px}
.ti-wp-row-day{font-size:12px;font-weight:700;color:var(--tia);text-transform:uppercase;letter-spacing:.5px}
.ti-wp-row-date{font-size:10px;color:var(--tid);margin-top:1px}
.ti-wp-row-ico{font-size:22px;line-height:1;text-align:center}
.ti-wp-row-temps{font-size:12px;font-family:var(--tim);font-variant-numeric:tabular-nums;white-space:nowrap;display:flex;gap:6px}
.ti-wp-row-temps .hi{color:var(--tio);font-weight:700}
.ti-wp-row-temps .lo{color:#60a5fa;font-weight:600}
.ti-wp-row-desc{font-size:11px;color:var(--tid);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0}
.ti-wp-row-times{font-size:10px;color:var(--tid);display:flex;flex-direction:column;gap:2px;text-align:right;font-variant-numeric:tabular-nums}
...
/* Widget card */
.ti-w{margin:12px;padding:16px;background:var(--tig);border:1px solid rgba(255,255,255,.08);border-radius:18px;transition:border-color .2s,background .2s}
...
.ti-pw-line{display:flex;justify-content:center;align-items:baseline;flex-wrap:wrap;gap:8px 12px;margin-bottom:8px;padding-bottom:8px;border-bottom:1px solid var(--tib)}
.ti-pw-l-time{font-family:var(--tim);font-size:15px;font-weight:700;color:var(--tit);letter-spacing:.5px}
.ti-pw-l-en{font-size:11px;color:var(--tid);direction:ltr}
.ti-pw-l-ar{font-size:12px;color:var(--tia2);font-family:var(--tiar);font-weight:600;direction:rtl}
.ti-pw-l-dot{width:3px;height:3px;border-radius:50%;background:var(--tib);align-self:center}
.ti-wt-row{justify-content:space-between}
.ti-habous-btn{display:inline-flex;align-items:center;gap:4px;padding:3px 10px;background:rgba(52,211,153,.1);border:1px solid rgba(52,211,153,.3);border-radius:20px;color:var(--tig2);text-decoration:none;font-size:10px;font-weight:600;text-transform:none;letter-spacing:0;transition:background .2s,transform .15s;cursor:pointer;font-family:var(--tif)}
...
/* Weather (compact) */
.ti-ww{background:linear-gradient(135deg,#3b82f6,#06b6d4);border-radius:14px;padding:12px;color:#fff;margin:10px;box-shadow:0 3px 12px rgba(0,0,0,.2)}
.ti-wwc{font-size:12px;font-weight:700;opacity:.95}
.ti-wwi{font-size:1.7rem}
.ti-wwd{font-size:11px;opacity:.9}
.ti-wwld{padding:16px;text-align:center;color:rgba(255,255,255,.7);font-size:11px}
.ti-ww-hr{display:flex;flex-direction:column;align-items:flex-end;gap:6px}
.ti-ww-ext{background:rgba(255,255,255,.18);border:1px solid rgba(255,255,255,.3);color:#fff;width:22px;height:22px;border-radius:7px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:12px;line-height:1;padding:0;transition:background .2s,transform .15s}
.ti-ww-ext:hover{background:rgba(255,255,255,.3);transform:translateY(-1px)}
.ti-ww-ext:active{transform:scale(.95)}
.ti-ww-today{display:grid;grid-template-columns:auto 1fr auto;gap:9px;align-items:center}
.ti-ww-today-ico{font-size:2.1rem;line-height:1;text-align:center;min-width:38px}
.ti-ww-today-main{min-width:0}
.ti-ww-today-tempRow{display:flex;align-items:baseline;gap:6px;margin-top:2px;flex-wrap:wrap}
.ti-ww-today-temp{font-size:1.4rem;font-weight:700;line-height:1;font-variant-numeric:tabular-nums}
.ti-ww-today-stats{display:flex;gap:8px;flex-wrap:wrap;font-size:10px;opacity:.95;margin-top:6px}
.ti-ww-today-stats span{white-space:nowrap}
.ti-ww-next{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:9px;padding-top:8px;border-top:1px solid rgba(255,255,255,.2)}
.ti-ww-day{background:rgba(255,255,255,.1);border-radius:9px;padding:6px 5px;text-align:center;display:flex;flex-direction:column;gap:2px;align-items:center}
.ti-ww-day-lbl{font-size:8.5px;opacity:.75;letter-spacing:.5px;font-weight:700}
.ti-ww-day-ico{font-size:1.4rem;line-height:1}
.ti-ww-day-temps{font-size:11px;font-variant-numeric:tabular-nums;display:flex;justify-content:center;gap:5px}
.ti-ww-day-temps .hi{font-weight:700}
.ti-ww-day-temps .lo{opacity:.75}
.ti-ww-day-desc{font-size:9px;opacity:.85;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%}
...
/* ── Life Calendar ── */
.ti-lc-motto{text-align:center;padding:10px 6px 12px;margin-bottom:6px;border-bottom:1px solid var(--tib)}
.ti-lc-motto-ar{direction:rtl;font-family:var(--tiar);font-size:14px;font-weight:600;color:var(--tia2);line-height:1.6;margin-bottom:6px}
.ti-lc-motto-num{font-family:var(--tim);font-size:32px;font-weight:700;color:var(--tip);line-height:1.1;text-shadow:0 0 14px rgba(175,82,222,.3)}
.ti-lc-motto-lbl{font-size:9px;color:var(--tid);text-transform:uppercase;letter-spacing:1.5px;margin-top:2px}
...
.ti-lc-grid-wrap{position:relative;overflow:hidden}
/* 10-stage DOM grid (renderLifeCalNew) */
.ti-lc-grid{display:grid;grid-template-columns:repeat(52,1fr);gap:0}
.ti-lc-wk{width:100%;height:4px;border-radius:1px;background:rgba(255,255,255,.08);transition:opacity .2s ease,transform .15s ease,box-shadow .2s ease;position:relative}
.ti-lc-wk.lived{opacity:1}
.ti-lc-wk.future{opacity:.15}
.ti-lc-wk.future:hover,.ti-lc-wk.lived:hover{opacity:.85;transform:scaleY(1.8);z-index:2}
.ti-lc-wk.cur{background:var(--tip)!important;box-shadow:0 0 12px 4px rgba(175,82,222,.5);border-radius:2px;opacity:1!important;transform:scaleY(2.2);z-index:3}
.ti-lc-wk.decade{border-top:1px solid rgba(255,255,255,.12)}
.ti-lc-wk.legal{border-left:2px dashed #fff!important;z-index:2}
.ti-lc-wk.legal::after{content:'18';position:absolute;top:-8px;left:-5px;font-size:5px;color:#fff;background:rgba(0,0,0,.65);padding:0 2px;border-radius:2px;font-weight:700;line-height:1.3;letter-spacing:.5px}
...
.ti-pal-cat{font-size:10px;color:var(--tid);padding:4px 8px 2px;text-transform:uppercase;letter-spacing:.06em}
  ...
  island.innerHTML=`
    <div class="ti-s" id="ti-s-wth" data-sec="wth" tabindex="0" role="button" aria-label="Weather — hover or click for forecast"><span class="ti-emoji" id="ti-wth-ico" style="font-size:14px;line-height:1">🌤️</span><span class="ti-wth" id="ti-wth">--</span></div>
    <div class="ti-d" data-sep="wth-date"></div>
    <div class="ti-s" id="ti-s-en" data-sec="date" tabindex="0" role="button" aria-label="Date — hover or click for calendar"><span class="ti-emoji" style="font-size:14px;line-height:1">📅</span><span class="ti-en" id="ti-en"></span></div>
    <div class="ti-d" data-sep="date-clk"></div>
    <div class="ti-s" id="ti-s-clk" data-sec="clk" tabindex="0" role="button" aria-label="Clock — hover or click for today's calendar"><span class="ti-dot"></span><span class="ti-clk" id="ti-clk"></span></div>
    <div class="ti-d" data-sep="clk-hijri"></div>
    <div class="ti-s" id="ti-s-ar" data-sec="hijri" tabindex="0" role="button" aria-label="Hijri date — hover or click for details"><span class="ti-emoji" style="font-size:14px;line-height:1">🌙</span><span class="ti-ar" id="ti-ar"></span></div>
    <div class="ti-d" data-sep="hijri-pray"></div>
    <div class="ti-s" id="ti-s-np" data-sec="pray" tabindex="0" role="button" aria-label="Next prayer countdown — hover or click for prayer times"><span class="ti-emoji" style="font-size:14px;line-height:1">🕌</span><span class="ti-np" id="ti-np"></span></div>
  ...
  sb.innerHTML=`
    ...
    <div class="ti-w" id="ti-pw">
      <div class="ti-wt ti-wt-row"><span>🕌 مواقيت الصلاة</span><a class="ti-habous-btn" id="ti-habous-btn" href="#" target="_blank" rel="noopener noreferrer">📅 الشهري / Monthly</a></div>
      ...
      <div class="ti-pw-line">
        <span class="ti-pw-l-en" id="ti-pw-date-en"></span>
        <span class="ti-pw-l-dot"></span>
        <span class="ti-pw-l-time" id="ti-pw-time"></span>
        <span class="ti-pw-l-dot"></span>
        <span class="ti-pw-l-ar" id="ti-pw-date-ar"></span>
      </div>
    ...
    <div class="ti-w" id="ti-lc-w" style="padding:12px;${cfg.showLifeCal?'':'display:none'}">
      ...
      <div class="ti-lc-motto">
        <div class="ti-lc-motto-ar">احرص علئ ما ينفعك استعن بالله ولاتعجز</div>
        <div class="ti-lc-motto-num" id="ti-lc-weeks-left">—</div>
        <div class="ti-lc-motto-lbl">Weeks Left</div>
      </div>
      <div class="ti-lc-row">
    ...
    <div class="ti-w" id="ti-settings">
      ...
      <div class="ti-set-row"><span class="ti-set-label">Use Old Life Calendar</span><button class="ti-set-tog ${cfg.useOldLifeCal?'on':'off'}" id="ti-tog-oldlifecal"></button></div>
      ...
      <div style="display:flex;flex-direction:column;gap:6px;font-size:12px">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center"><span style="color:var(--tid)">Change City</span><span><span class="ti-kbd">Alt</span>+<span class="ti-kbd">C</span></span></div>
      </div>
    </div>
    ...
    <div class="ti-w" id="ti-lnk-w"><div class="ti-wt">🔗 Quick Links</div>
      <div class="ti-lnk-wrap">
        ...
        <div id="ti-lnk-err" style="display:none;color:#f87171;font-size:10px"></div>
      </div>
    </div>

    <div class="ti-foot">🏝️ Time Island v5.0.0 (Hybrid)</div>`;
  ...
  const R={clk:$('ti-clk'),en:$('ti-en'),ar:$('ti-ar'),np:$('ti-np'),pwTime:$('ti-pw-time'),pwDateEn:$('ti-pw-date-en'),pwDateAr:$('ti-pw-date-ar'),pg:$('ti-pg'),pcd:$('ti-pcd'),sc:$('ti-sc'),dig:$('ti-dig'),sw:$('ti-sw'),notes:$('ti-notes'),ww:$('ti-ww'),ci:$('ti-ci'),cd:$('ti-cd'),age:$('ti-age'),habous:$('ti-habous-btn'),wth:$('ti-wth')};
  ...
  function renderDD(q=''){
    ...
    if(!filtered.length){R.cd.innerHTML='<div style="padding:12px;text-align:center;color:var(--tid);font-size:11px">No results</div>';}
      ...
      R.cd.innerHTML=filtered.map((c,i)=>{const cur=CITIES[selCity];const isSel=cur&&c[0]===cur[0];return `<div class="ti-csdi${isSel?' sel':''}${i===hlIdx?' hl':''}" data-i="${CITIES.indexOf(c)}">${c[1]} - ${c[2]}</div>`}).join('');
      if(hlIdx>=0)R.cd.querySelector('.ti-csdi.hl')?.scrollIntoView({block:'nearest'});
    }
  ...
  R.ci.addEventListener('input',()=>{
    // Auto-highlight the first match once there's an actual query, so Enter
    // immediately picks it. Nothing highlighted on an empty query — there's
    // no single "best" match yet.
    hlIdx=R.ci.value.trim()?0:-1;
    renderDD(R.ci.value);
  });
  ...
  function fetchPrayer(){
    ...
    const ds=`${P(now.getDate())}-${P(now.getMonth()+1)}-${now.getFullYear()}`;
    // method=21 is Aladhan's Morocco/Habous preset, but it's still an
    // astronomical approximation and can drift a minute or two from the
    // ministry's published table (habous.gov.ma). `tune` (order: Imsak,Fajr,
    // Sunrise,Dhuhr,Asr,Maghrib,Sunset,Isha,Midnight) nudges each prayer by a
    // constant number of minutes. These offsets are a majority-vote fit
    // across 16 cross-checked dates (Marrakech, 16-31 Jul 2026, habous.gov.ma
    // vs Aladhan's own uncalibrated method=21 for the same dates/city):
    const TUNE='0,0,0,0,0,0,0,0,0';
    const url=`https://api.aladhan.com/v1/timings/${ds}?latitude=${c[4]}&longitude=${c[5]}&method=21&tune=${TUNE}`;
    const myId=++prayerReqId;
    GM_xmlhttpRequest({method:'GET',url,timeout:10000,onload(r){
      if(myId!==prayerReqId)return; // superseded by a newer request (city switched)
      try{
        ...
        R.pwDateAr.textContent=prayerHijri;
        R.pwDateEn.textContent=enDateStr(now);
        renderPG();
      }catch{R.pg.innerHTML='<div class="ti-pld" style="color:var(--tir)">خطأ في التحميل</div>'}
    },onerror(){if(myId===prayerReqId)R.pg.innerHTML='<div class="ti-pld" style="color:var(--tir)">خطأ في الاتصال</div>'},
    ontimeout(){if(myId===prayerReqId)R.pg.innerHTML='<div class="ti-pld" style="color:var(--tir)">انتهت مهلة الاتصال</div>'}});
  }
  ...
  function dayLabel(di,dateObj){
    return di===0?'Today':di===1?'Tomorrow':EN_D[dateObj.getDay()];
  }
  function dayShortDesc(day){
    return day?.hourly?.[4]?.weatherDesc?.[0]?.value || day?.hourly?.[2]?.weatherDesc?.[0]?.value || '';
  }
  function renderWeatherCard(){
    if(!weatherData)return '<div class="ti-wwld">Weather unavailable</div>';
    const w=weatherData;
    const todayIco=pickWeatherEmoji(w.desc);
    const nextDays=(w.forecast||[]).slice(1,3);
    const nextLabels=['TOMORROW','AFTER TOMORROW'];
    let nextHtml='';
    for(let i=0;i<2;i++){
      const day=nextDays[i];
      if(!day){
        nextHtml+=`<div class="ti-ww-day"><div class="ti-ww-day-lbl">${nextLabels[i]}</div><div class="ti-ww-day-ico">🌤️</div><div class="ti-ww-day-desc">—</div></div>`;
        continue;
      }
      const desc=dayShortDesc(day);
      const ico=pickWeatherEmojiPlain(desc);
      nextHtml+=`<div class="ti-ww-day">
        <div class="ti-ww-day-lbl">${nextLabels[i]}</div>
        <div class="ti-ww-day-ico">${ico}</div>
        <div class="ti-ww-day-temps"><span class="hi">▲ ${escHtml(day.maxtempC)}°</span><span class="lo">▼ ${escHtml(day.mintempC)}°</span></div>
        <div class="ti-ww-day-desc" title="${escHtml(desc)}">${escHtml(desc||'—')}</div>
      </div>`;
    }
    return `
      <div class="ti-ww-today">
        <div class="ti-ww-today-ico">${todayIco}</div>
        <div class="ti-ww-today-main">
          <div class="ti-wwc">${escHtml(w.cityEn)}</div>
          <div class="ti-ww-today-tempRow">
            <span class="ti-ww-today-temp">${escHtml(w.temp)}°C</span>
            <span class="ti-wwd">${escHtml(w.desc)}</span>
          </div>
          <div class="ti-ww-today-stats">
            <span>💧 ${escHtml(w.humidity)}%</span>
            <span>💨 ${escHtml(w.wind)} km/h</span>
            <span>🌡️ ${escHtml(w.feels)}°C</span>
          </div>
        </div>
        <div class="ti-ww-hr">
      ...
      <div class="ti-ww-next">${nextHtml}</div>`;
  }
  ...
  function updIslandWeather(){
    const icoEl=document.getElementById('ti-wth-ico');
    if(!weatherData){R.wth.textContent='--';if(icoEl)icoEl.textContent='🌤️';return}
    if(icoEl)icoEl.textContent=pickWeatherEmoji(weatherData.desc);
  ...
  function renderWthPop(){
    ...
    if(!days.length){
      ...
      // Today: detailed 4-slot block
      const today=days[0];
      const tObj=new Date(today.date+'T00:00:00');
      const tDateStr=`${EN_DS[tObj.getDay()]} · ${EN_M[tObj.getMonth()]} ${tObj.getDate()}`;
      h+=`<div class="ti-wp-day">
        <div class="ti-wp-day-hdr">
          <div><span class="ti-wp-day-name">Today</span><span class="ti-wp-day-date">${tDateStr}</span></div>
          <div class="ti-wp-day-temps"><span class="hi">▲ ${escHtml(today.maxtempC)}°</span><span class="lo">▼ ${escHtml(today.mintempC)}°</span></div>
        </div>
        <div class="ti-wp-slots">`;
      WTH_SLOTS.forEach(s=>{
        const hr=today.hourly&&today.hourly[s.idx];
      ...
      // Tomorrow + After-tomorrow: compact rows
      for(let di=1;di<days.length;di++){
        const day=days[di];
        const dObj=new Date(day.date+'T00:00:00');
        const dayName=di===1?'TOMORROW':EN_D[dObj.getDay()].toUpperCase();
        const dateStr=`${EN_DS[dObj.getDay()]} · ${EN_M[dObj.getMonth()]} ${dObj.getDate()}`;
        const desc=dayShortDesc(day)||'N/A';
        const ico=pickWeatherEmojiPlain(desc);
        const morning=day.hourly?.[2];
        const evening=day.hourly?.[6];
        h+=`<div class="ti-wp-row">
          <div class="ti-wp-row-when">
            <div class="ti-wp-row-day">${dayName}</div>
            <div class="ti-wp-row-date">${dateStr}</div>
          </div>
          <div class="ti-wp-row-ico">${ico}</div>
          <div class="ti-wp-row-temps"><span class="hi">▲ ${escHtml(day.maxtempC)}°</span><span class="lo">▼ ${escHtml(day.mintempC)}°</span></div>
          <div class="ti-wp-row-desc" title="${escHtml(desc)}">${escHtml(desc)}</div>
          <div class="ti-wp-row-times">
            <span>🌅 ${morning?escHtml(morning.tempC)+'°':'—'}</span>
            <span>🌇 ${evening?escHtml(evening.tempC)+'°':'—'}</span>
          </div>
        </div>`;
      }
    }
  ...
  function fetchWeather(){
    ...
    const myId=++weatherReqId;
    GM_xmlhttpRequest({method:'GET',url:`https://wttr.in/${city}?format=j1`,timeout:8000,onload(r){
      if(myId!==weatherReqId)return; // superseded by a newer request (city switched)
      try{
        const d=JSON.parse(r.responseText);const cur=d.current_condition[0];
        const desc=cur.weatherDesc?.[0]?.value||'N/A';
        const ico=pickWeatherEmoji(desc);
  ...
  function hoverSetup(trigId,popup,renderFn,posRight){
    ...
    // Keyboard access: triggers got tabindex/role="button" for a11y — without
    // this they'd be focusable but inert. Enter/Space opens/closes like a click.
  ...
  //  §11b  LIFE CALENDAR
  // ═══════════════════════════════════════════
  const lcBody=$('ti-lc-body'), lcBday=$('ti-lc-bday'), lcExp=$('ti-lc-exp'), lcWeeksLeftEl=$('ti-lc-weeks-left');

  function renderLifeCal(){
    // Weeks Left motto lives outside #ti-lc-body and is shared by both grid
    // styles below, so it's kept in sync here regardless of which renders.
    if(!cfg.lifeBday){
      lcWeeksLeftEl.textContent='—';
    }else{
      const bday0=new Date(cfg.lifeBday+'T00:00:00');
      if(isNaN(bday0.getTime())){
        lcWeeksLeftEl.textContent='—';
      }else{
        const msLived0=new Date().getTime()-bday0.getTime();
        if(msLived0<0){
          lcWeeksLeftEl.textContent='—';
        }else{
          const weeksLived0=Math.floor(msLived0/(7*24*60*60*1000));
          lcWeeksLeftEl.textContent=Math.max(0,cfg.lifeExpect*52-weeksLived0).toLocaleString();
        }
      }
    }
    if(cfg.useOldLifeCal) renderLifeCalOld(); else renderLifeCalNew();
  }

  // 10-stage neurological/Islamic chronology grid (default).
  function renderLifeCalNew(){
    if(!cfg.lifeBday){
    ...
    const curStage=lifeStage(ageYrs);
    h+=`<div style="text-align:center;font-size:10px;color:var(--tia2);margin:2px 0 6px;letter-spacing:.5px;font-weight:600">🧬 Stage: <span style="color:var(--tit)">${escHtml(curStage.label)}</span></div>`;

    h+='<div class="ti-lc-grid-wrap"><div class="ti-lc-grid">';
    const legalWeek=18*52; // exact week index of the 18th birthday
    for(let yr=0;yr<exp;yr++){
      const isDec=yr>0&&yr%10===0;
      const color=lifeStage(yr).color;
      for(let wk=0;wk<52;wk++){
        const w=yr*52+wk;
        let cls = w < weeksLived ? 'lived' : (w === weeksLived ? 'cur' : 'future');
        if(isDec) cls+=' decade';
        if(w===legalWeek) cls+=' legal';
        h+=`<div class="ti-lc-wk ${cls}" style="background:${color}" title="Age ${yr}, Week ${wk+1} — ${escHtml(lifeStage(yr).label)}"></div>`;
      }
    }
    h+='</div></div>';

    h+='<div class="ti-lc-legend" style="flex-wrap:wrap;justify-content:center;gap:4px 8px;margin-top:6px;padding:4px 0">';
    LIFE_STAGES.forEach(stage=>{
      h+=`<span style="display:inline-flex;align-items:center;gap:3px;font-size:7.5px;color:var(--tid);letter-spacing:.3px"><span style="display:inline-block;width:8px;height:8px;background:${stage.color};border-radius:1px;border:1px solid rgba(255,255,255,.1)"></span>${escHtml(stage.label)}</span>`;
    });
    h+='<span style="display:inline-flex;align-items:center;gap:3px;font-size:7.5px;color:var(--tid)"><span style="display:inline-block;width:8px;height:8px;border-left:2px dashed #fff;background:transparent"></span>18 Legal</span>';
    h+='</div>';

    lcBody.innerHTML=h;
  }

  // Old canvas-based Life Calendar (plain lived/now/future, no per-stage
  // coloring) — kept as the previously-published implementation, selectable
  // via the "Use Old Life Calendar" setting.
  function renderLifeCalOld(){
    if(!cfg.lifeBday){
      lcBody.innerHTML='<div class="ti-lc-prompt">Enter your birthday above to see<br>your life in weeks</div>';
      return;
    }
  ...
  setupTog('ti-tog-oldlifecal','useOldLifeCal',()=>{renderLifeCal()});
  ...
  function addLink(){
    const nameEl=$('ti-lnk-name'),urlEl=$('ti-lnk-url'),errEl=$('ti-lnk-err');
    const n=nameEl.value.trim();
    let u=urlEl.value.trim();
    errEl.style.display='none';
    if(!n||!u)return;
    if(!/^[a-z][a-z0-9+.-]*:/i.test(u)) u='https://'+u; // bare "example.com" -> assume https
    if(!isSafeUrl(u)){
      errEl.textContent='Only http:// and https:// links are allowed.';
      errEl.style.display='';
      return;
    }
    userLinks.push({n,u});
  ...
  function tick(){
    try{
      ...
      if(s!==lastS){
        ...
        R.en.textContent=enDateStr(now);
      ...
      // setInterval already survives a throwing callback (unlike the old
      // self-rescheduling rAF loop, which would freeze forever on error) —
      // this catch just keeps the failure visible and scoped to one tick.
      console.error('[Time Island] tick() error (skipping this frame):',e);
    }
  }
  ...
  /** Alt+C: jump straight to the city search box — opens the sidebar if it's
   *  closed, scrolls the prayer widget into view, focuses the input, selects
   *  its current text (so typing immediately replaces it), and opens the
   *  dropdown. Lets a user retype a city without ever touching the mouse. */
  function focusCitySearch(){
    const wasOpen=sb.classList.contains('open');
    sb.classList.add('open');
    $('ti-pw').scrollIntoView({block:'start',behavior:'smooth'});
    setTimeout(()=>{
      R.ci.focus();
      R.ci.select();
      hlIdx=-1;
